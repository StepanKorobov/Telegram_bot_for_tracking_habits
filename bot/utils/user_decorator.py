...
from bot.database.models import get_user_by_telegram_id, check_user_by_telegram_id
...
def get_current_user_from_inline_button(func):
    ...
    def wrapper(call, *args, **kwargs):
        current_user = get_user_by_telegram_id(telegram_id=call.from_user.id)
...
def check_user_registration(func):
    ...
    @wraps(func)
    def wrapper(message, *args, **kwargs):
        user_exist = check_user_by_telegram_id(telegram_id=message.from_user.id)
        if user_exist:
            # return func(message, current_user=current_user, *args, **kwargs)
