...
def get_graphs_habits(data: list) -> BytesIO:
    """
    Создание графика на основе статистики привычек.
    По оси X - даты, Y - время выполнения привычек.

    Args:
        Список привычек со статистикой.

    Returns:
        Буфер содержащий картинку с графиком статистики.
    """
    ...
    for idx, habit in enumerate(habits):
        ...
        if len(check_dates) > 0:
            ...
            ax.set_title(
                f"{habit_name} — всего выполнений: {len(check_dates)}",
                fontsize=12,
                fontweight="bold",
            )
            ...
            ax.set_title(
                f"{habit_name} — всего выполнений: 0", fontsize=12, fontweight="bold"
            )
            ax.set_xlim(0, 1)
            ax.set_ylim(0, 24)

    fig.suptitle(
        "Статистика: время выполнения привычек по датам",
        fontsize=16,
        fontweight="bold",
        y=1.02,
    )
    ...
    img_buffer = BytesIO()
    plt.savefig(
        img_buffer, format="png", dpi=150, bbox_inches="tight", facecolor="white"
    )
