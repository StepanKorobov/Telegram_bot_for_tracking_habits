...
def get_graphs_habits(data):
    ...
    fig, axes = plt.subplots(n_habits, 1, figsize=(14, 4 * n_habits))

    if n_habits == 1:
        axes = [axes]

    colors = [f"#{random.randint(0, 0xFFFFFF):06X}" for _ in range(n_habits)]

    for idx, habit in enumerate(habits):
        habit_name = habit['habit_name']
        check_dates = habit['habits_check_date']
